...
- **API Gateway**: Exposes `/actuator/health` with detailed status (Spring Boot Actuator)
- **Account Service (NestJS)**: Added `/health` endpoint returning "OK" status
- **All Spring Boot services**: Use Spring Boot Actuator with `management.endpoint.health.show-details=always`
- **All services**: Return HTTP 200 for healthy, HTTP 503 for unhealthy
- **Health endpoint examples**:
  ```bash
  # Test API Gateway health
  curl http://localhost:8080/actuator/health
  # Expected response: JSON with detailed status
